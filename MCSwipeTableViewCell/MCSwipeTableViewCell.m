...
- (void)setupSwipingView {
    ...
    UIView *tmp = [self copy];
    UIImage *contentViewScreenshotImage = [self imageWithView:tmp];
...
{
    MCSwipeTableViewCell *copy = [[[self class] alloc] init];
    if (copy) {
        copy.imageView.image = self.imageView.image;
        copy.textLabel.text = self.textLabel.text;
